...
async def process_referral_reward(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Grants the referral reward only when the new user passes verification."""
        ...
        if ref_id and not claimed:
            ...
            try:
                await context.bot.send_message(
                    ref_id,
                    f"🎉 <b>New Referral!</b>\nA user successfully joined using your link. You earned {REFERRAL_BONUS} BDT!",
                    parse_mode="HTML",
                )
            except Exception:
                pass  
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Main Menu Routing
        ...
        if data == "menu_balance":
            ...
            task_text = (
                "📋 <b>Complete these tasks to earn 4 BDT!</b>\n\n"
                "Join the following groups/channels:\n"
                "Task 1: https://t.me/+Q7Se9P-2dvo2ZDhl\n"
                "Task 2: https://t.me/+XUrMlX-1_XRiODc1\n"
                "Task 3: https://t.me/+IwhZorG45tFhMGVl\n"
                "Task 4: https://t.me/+kv6SCJrAQpQ1Njg1\n\n"
                "<i>Click the Verify button after joining to claim your reward. You can check your Balance from the menu to see the added amount.</i>"
            )
            ...
            await query.edit_message_text(task_text, parse_mode="HTML", reply_markup=task_kb, disable_web_page_preview=True)
    ...
    # 3. Task Verification Process
    elif data == "verify_tasks":
        is_completed = await db.get_user_task_status(user_id)
        if is_completed:
            await query.answer("❌ You have already completed and claimed this reward!", show_alert=True)
        else:
            await db.update_balance(user_id, TASK_REWARD)
            await db.mark_tasks_completed(user_id)
            await query.edit_message_text(
                f"🎉 <b>Tasks Verified!</b>\n{TASK_REWARD} BDT has been successfully added to your balance.",
                parse_mode="HTML",
                reply_markup=main_menu()
            )
...
def main():
    ...
    logger.info("🚀 Bot Architecture v1.4.0 Initialized & Running...")
    app.run_polling()
