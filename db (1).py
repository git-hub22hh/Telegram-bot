...
async def init_db():
    """Initializes the database schemas and handles migrations."""
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Migration: Safely add 'tasks_completed' column to existing DB
        try:
            await db.execute("ALTER TABLE users ADD COLUMN tasks_completed BOOLEAN DEFAULT FALSE")
...
# --- New Task Verification Functions ---
async def get_user_task_status(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT tasks_completed FROM users WHERE user_id = ?", (user_id,)) as cursor:
            res = await cursor.fetchone()
            return res[0] if res else False

async def mark_tasks_completed(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET tasks_completed = TRUE WHERE user_id = ?", (user_id,))
        await db.commit()
